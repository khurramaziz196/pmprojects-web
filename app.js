...
const WORKSPACE_CACHE_VERSION = 11;
...
function projectHierarchyRows(projects) {
    ...
    projects.forEach(project => {
        const parentId = resolvedProjectParentId(project, projectIds, projects);
...
function resolvedProjectParentId(project, projectIds, projects) {
    const explicitParentId = project.parent_project_id || "";
    if (explicitParentId && projectIds.has(explicitParentId)) {
        return explicitParentId;
    }

    const projectPO = String(project.po_number || "").trim();
    const projectCustomer = String(project.customer || "").trim().toLowerCase();
    const projectRig = String(project.rig_number || "").trim().toLowerCase();
    const projectStatus = String(project.status || "").trim().toLowerCase();
    if (projectPO || !projectCustomer || !projectRig || !projectStatus) {
        return "";
    }

    const parentCandidates = projects
        .filter(candidate => (
            candidate.id !== project.id
            && String(candidate.po_number || "").trim()
            && String(candidate.customer || "").trim().toLowerCase() === projectCustomer
            && String(candidate.rig_number || "").trim().toLowerCase() === projectRig
            && String(candidate.status || "").trim().toLowerCase() === projectStatus
        ))
        .sort((left, right) => {
            const leftOrder = Number.isFinite(Number(left.sort_order)) ? Number(left.sort_order) : Number.MAX_SAFE_INTEGER;
            const rightOrder = Number.isFinite(Number(right.sort_order)) ? Number(right.sort_order) : Number.MAX_SAFE_INTEGER;
...
function progressForProject(project) {
    ...
    const childProjects = state.projects.filter(candidate => resolvedProjectParentId(candidate, projectIds, state.projects) === project.id);
